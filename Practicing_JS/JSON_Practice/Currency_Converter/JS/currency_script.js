document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelector('#currency_form').onsubmit = () => {
    ...
    let base_currency = document.querySelectory('#base_currency').value
    let api_endpoint = `${api_endpoint_url}$base=${base_currency}`

    const request = new XMLHttpRequest();
    request.open('GET', api_endpoint);

    console.log(document.querySelector("#base_currency").value);
    console.log(document.querySelector("#to_currency").value);
